...
function decodeWordPressHtml(value: string) {
  return value
    .replace(/<[^>]*>/g, '')
    .replace(/&#8217;|&#x2019;/g, "'")
    .replace(/&#8211;|&#x2013;/g, '-')
...
function mapWordPressPost(post: WordPressPost): BlogPost {
  ...
  return {
    ...
    excerpt: decodeWordPressHtml(post.excerpt.rendered),
    content: decodeWordPressHtml(post.content.rendered),
    contentHtml: post.content.rendered,
    author: post._embedded?.author?.[0]?.name ?? 'Devhulon Business & People Solutions',
    category: terms.find((term) => term.name)?.name ?? 'Insights',
    date: post.date,
...
export async function getWordPressPosts(): Promise<BlogPost[]> {
  ...
  return Array.isArray(posts) ? posts.map(mapWordPressPost) : []
}
...
export async function getWordPressPost(slug: string) {
  ...
  return Array.isArray(posts) && posts.length > 0 ? mapWordPressPost(posts[0]) : undefined
}
